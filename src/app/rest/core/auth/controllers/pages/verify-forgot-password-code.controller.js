...
const VerifyForgotPasswordCodeController = async (args)=>{
  ...
  let meta={
    title:"Forgot Password Verify Code | Agglomy !",
    meta: [
      {
        name: "child value",
    ...
    async verifyCode(){
      ...
      try {
        ...
        if(validation === 'ok'){
          ...
          let mUser={email:this.user.email,passwordCode:this.user.code}
          let response= await client?.auth().verifyPassCode(mUser).val()
          if(response?.error){
...
export default VerifyForgotPasswordCodeController
